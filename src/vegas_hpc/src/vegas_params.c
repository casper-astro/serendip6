...
void set_obs_params_gb(char *buf, 
    ...
    if (strcmp("VEGAS", p->hdr.backend)==0)
        p->hdr.be_phase = -1;
    else 
        p->hdr.be_phase = -1;
...
void vegas_read_net_params(char *buf, struct vegas_udp_params *u) {
    ...
    get_str("PKTFMT", u->packet_format, 32, "VEGAS");
                           ...
                           struct psrfits *p)
{
    ...
    get_str("BACKEND", p->hdr.backend, 24, "VEGAS");
    ...
    vegas_read_subint_params(buf, g, p);
