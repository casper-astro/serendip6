    ...
    if (rv==-1) { return(VEGAS_ERR_SYS); }
    	...
    	if (strncmp(p->packet_format, "SPEAD", 5) == 0)
	    {
    	    if (!bw)    /* only for lbw */
	        {
	            ...
	            char *in = tempbuf+8;           /* skip 8-byte header */
	            char *out = b->data + 72;       /* skip 72-byte header */
                (void) memcpy(out, in, 8192);   /* copy data and ignore the last 3 words (frame error count, inter-frame gaps) */
    	    }
