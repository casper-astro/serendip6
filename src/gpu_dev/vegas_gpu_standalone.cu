...
int Init()
{
    ...
    /* check if the data buffer is large enough to contain at least one
       [PFB +] transform */
    if (g_iSizeRead < (g_iNumSubBands * g_iNTaps * g_iNFFT * NUM_BYTES_PER_SAMP))
    {
        int iMaxNFFT = g_iSizeRead / (g_iNumSubBands * g_iNTaps * NUM_BYTES_PER_SAMP);
        (void) fprintf(stderr,
                       "ERROR: Cannot perform specified operation with given "
                       "input buffer size! Input buffer size is %d MB, "
                       "maximum length of FFT is %d.\n",
                       (int) ((float) g_iSizeRead) / (1024 * 1024),
                       iMaxNFFT);
        return EXIT_FAILURE;
    }
    ...
    if (g_iIsPFBOn)
    {
        lTotCUDAMalloc += (g_iNumSubBands * g_iNTaps * g_iNFFT * sizeof(float));
    }
    lTotCUDAMalloc += g_iSizeRead;
    lTotCUDAMalloc += (g_iNumSubBands * g_iNFFT * sizeof(float4));
    lTotCUDAMalloc += (g_iNumSubBands * g_iNFFT * sizeof(float4));
    lTotCUDAMalloc += (g_iNumSubBands * g_iNFFT * sizeof(float4));
    ...
    else
    {
        ...
        if (g_iIsPFBOn)
        {
            (void) printf("    PFB coefficients: %d MB\n",
                          ((float) g_iNumSubBands * g_iNFFT * sizeof(float4)) / (1024 * 1024));
        }
    }
#endif
